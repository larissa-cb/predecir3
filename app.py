...
if app_mode == "Predicción Individual":
    ...
    fig = px.line(trend_data, x='Semestre', y=['Tasa Deserción', 'Intervenciones'],
                 title='Evolución de la Deserción vs Intervenciones',
    ...
    fig = px.pie(factors_data, values='Impacto', names='Factor',
                title='Distribución de Factores de Deserción')
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(career_data, x='Carrera', y=['Alto Riesgo', 'Riesgo Medio', 'Bajo Riesgo'],
                title='Distribución de Riesgo por Carrera',
                labels={'value': 'Cantidad de Estudiantes', 'variable': 'Nivel de Riesgo'})
    st.plotly_chart(fig, use_container_width=True)
    ...
    **Sistema inteligente** para la identificación temprana de estudiantes en riesgo 
    ...
    - **Algoritmos**: Modelos predictivos basados en machine learning
    ...
    ### 📞 Soporte y Contacto
    Para soporte técnico o implementación personalizada:
    - 📧 Email: soporte@universidad.edu
    - 📞 Teléfono: +34 900 123 456
    - 🏢 Oficina: Vicerrectorado de Estudiantes
    """)
...
st.markdown("---")
st.caption("© 2024 Sistema de Predicción de Deserción Universitaria | Desarrollado con Streamlit")